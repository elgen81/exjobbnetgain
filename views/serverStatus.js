    ...
    axios_1["default"].get('http://127.0.0.1:' + _config_1.config.port + '/menu/getStatus').then(function (res) {
        if (res.status == 200) {
            switch (res.data.DBStatus) {
                case 0:
                    document.getElementById("statusS").innerText = "Server status: Disconnected";
                    document.getElementById("statusS").style.color = "red";
                    break;
                case 1:
                    document.getElementById("statusS").innerText = "Server status: Connected";
                    document.getElementById("statusS").style.color = "green";
                    break;
                case 2:
                    document.getElementById("statusS").innerText = "Server status: Connecting";
                    document.getElementById("statusS").style.color = "yellow";
                    break;
                case 3:
                    document.getElementById("statusS").innerText = "Server status: Disconnecting";
                    document.getElementById("statusS").style.color = "yellow";
                    break;
                default:
                    document.getElementById("statusS").innerText = "Server status: Error";
                    document.getElementById("statusS").style.color = "red";
                    break;
            }
        }
        ...
        document.getElementById("statusS").innerText = "Server status: Server Not Found";
        document.getElementById("statusS").style.color = "red";
    });
}
function statusQ() {
    ...
    QueueList.find({
        ...
        if (err) {
            ...
            document.getElementById("statusQ").innerText = "Active Queues : " + count;
        }
